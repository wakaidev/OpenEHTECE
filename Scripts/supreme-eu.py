...
def add_to_cart(soup, url):
	...
	if form is not None:
		...
		if soup.find('span', {'class': 'in_cart_text'}) is not None:
			checkout(session)
		else:
			sys.exit('Unable to add {} to cart'.format(product_name))
	else:
		sys.exit('Sorry, product is sold out!')
...
def checkout(session):
	...
	elif card_type.lower() == 'solo':
		card_ = 'solo'
	else:
		sys.exit('You must be using a master, visa, solo, or american express card')
